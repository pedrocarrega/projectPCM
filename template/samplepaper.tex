...
int\ast matrix_pointer_device;

cudaMalloc(&matrix_pointer_device, sizeof(int) \ast graph_size \ast graph_size):
...
cudaMemCpy(matrix_pointer_device, graph, sizeof(int) \ast graph_size \ast graph_size, cudaMemcpyHostToDevice);

Definition of the parameters that describe the kernel grid
dim3 threads(16,32);the number of threads per blocks should be multiple of 32
...
cudaMemCpy(graph, matrix_pointer_device, sizeof(int) \ast graph_size \ast graph_size, cudaMemcpyDeviceToHost);
